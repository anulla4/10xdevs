...
export class ObservationFormModal {
  ...
  constructor(page: Page) {
    ...
    this.dialog = page.getByTestId('observation-form-dialog').or(page.getByRole('dialog'));
    this.overlay = page.getByTestId('observation-form-overlay').or(page.locator('[data-slot="dialog-overlay"]'));
    this.content = page.getByTestId('observation-form-content').or(page.locator('form').first());
    this.form = page.getByTestId('observation-form').or(page.locator('form').first());

    this.name = page.getByTestId('field-name').or(page.locator('input[name="name"]')).or(page.getByLabel(/nazwa/i));
    ...
    this.observationDate = page
      .getByTestId('field-observation-date')
      .or(page.locator('input[name="observation_date"]'))
      .or(page.getByLabel(/data obserwacji/i));
    this.locationLat = page
      .getByTestId('field-location-lat')
      .or(page.locator('input[name="location_lat"]'))
      .or(page.getByLabel(/szerokość/i));
    this.locationLng = page
      .getByTestId('field-location-lng')
      .or(page.locator('input[name="location_lng"]'))
      .or(page.getByLabel(/długość/i));
    this.locationSource = page.getByTestId('field-location-source').or(page.locator('select[name="location_source"]'));
    this.isFavorite = this.form
      .getByTestId('field-is-favorite')
      .or(this.form.locator('input[name="is_favorite"]'))
      .or(this.form.getByRole('checkbox', { name: /dodaj do ulubionych/i }));

    this.cancelButton = page.getByTestId('btn-cancel-observation').or(page.getByRole('button', { name: /anuluj/i }));
    this.saveButton = page
      .getByTestId('btn-save-observation')
