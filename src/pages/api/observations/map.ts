...
export const GET: APIRoute = async (context) => {
  ...
  try {
    ...
    const bboxProvided = [min_lat, min_lng, max_lat, max_lng].some((v) => v !== undefined);
    const bboxComplete = [min_lat, min_lng, max_lat, max_lng].every((v) => v !== undefined);

    if (bboxProvided && !bboxComplete) {
      throw new ValidationError('Bounding box requires all four coordinates: min_lat, min_lng, max_lat, max_lng');
    }

    // Validate bbox ranges
    if (bboxComplete && (min_lat! >= max_lat! || min_lng! >= max_lng!)) {
      throw new UnprocessableError('Invalid bounding box: min values must be less than max values');
    }
    ...
    if (err instanceof ValidationError || err instanceof UnauthorizedError || err instanceof UnprocessableError) {
